...
class Sam(nn.Module):
    ...
    def forward_sam_with_embeddings(self, image_embeddings, points=None, masks=None, boxes=None):
        if points is not None:
            if not isinstance(points, (tuple, list)):
                if points.ndim == 2:
                    points = points.unsqueeze(1)
                # B *
                points = (points, torch.ones(points.size(0), 1).to(points))
        ...
        if masks is not None:
            dense_prompt_embeddings = self.prompt_encoder(
                points=None,
                boxes=None,
                masks=masks,
            )[1]
        ...
        #     sparse_prompt_embeddings=sparse_embeddings,
        #     dense_prompt_embeddings=dense_embeddings,
        #     expand_image_batch=False,
        # )
        image_pe = self.prompt_encoder.get_dense_pe()
        """Predicts masks. See 'forward' for more details."""
        # Concatenate output tokens
        output_tokens = torch.cat([self.mask_decoder.iou_token.weight,
        ...
        low_res_masks = (hyper_in @ upscaled_embedding.view(b, c, h * w)).view(b, -1, h, w)

        # Generate mask quality predictions
        iou_predictions = self.mask_decoder.iou_prediction_head(iou_token_out)

        from ..utils.amg import batched_mask_to_box
        boxes = batched_mask_to_box((F.interpolate(low_res_masks, size=1024).squeeze(1) > 0.0).bool())
        # boxes = batched_mask_to_box((low_res_masks.squeeze(1) > 0.0).bool()) * 4

        outputs = {
            'pred_logits': low_res_masks,
            'pred_boxes': boxes.float(),
            'pred_ious': iou_predictions,
            'iou_tokens': iou_token_out,
        }
